...
function AdminResources() {
  ...
  const statusClass = { ACTIVE: 'bg-green-100 text-green-700', OUT_OF_SERVICE: 'bg-red-100 text-red-700' }
  ...
  return (
    <div className="admin-bg min-h-screen py-10 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        {message && <div className="bg-green-50 border border-green-300 text-green-700 px-4 py-3 rounded-lg text-sm mb-5">{message}</div>}
        ...
        {showForm && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                { label: 'Name',     name: 'name',     type: 'text',   placeholder: 'e.g. Lab A101' },
                { label: 'Capacity', name: 'capacity', type: 'number', placeholder: 'e.g. 30' },
                { label: 'Location', name: 'location', type: 'text',   placeholder: 'e.g. Block A' },
              ].map(f => (
              ...
              <div className="flex items-end">
                <button type="submit" className="w-full bg-blue-600 text-white font-bold px-5 py-2.5 rounded-lg hover:bg-blue-700 transition text-sm">
                  Save Resource
                </button>
              </div>
            </form>
