...
function MyBookings() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      setAllBookings(res.data); setFiltered(res.data)
  ...
  const handleCancel = async (id) => {
    ...
    try {
      ...
      setAllBookings(updated); applyFilters(userId, statusFilter)
    ...
    <div className="admin-bg min-h-screen py-10 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm px-5 py-4 flex flex-wrap gap-3 items-center mb-6">
          <input type="number" placeholder="Filter by User ID..." value={userId}
          ...
          <select value={statusFilter} onChange={e => { setStatus(e.target.value); applyFilters(userId, e.target.value) }}
          ...
          <button onClick={() => { setUserId(''); setStatus('ALL'); setFiltered(allBookings) }}
            className="bg-gray-100 text-gray-700 font-semibold px-4 py-2 rounded-lg text-sm hover:bg-gray-200 transition">Clear</button>
